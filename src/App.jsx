...
/* ════════════════════════════════════════════════════════════════
   AI CHAT WIDGET — "IA Financiera" con nombre aleatorio
   Anthropic API · Búsqueda web en tiempo real
   Acceso: contraseña 1234 · WhatsApp: +54 11 4050-0087
════════════════════════════════════════════════════════════════ */
const IA_NAMES = ["Valeria","Sofía","Camila","Alexia","Natalia","Lumia","Aria","Celia","Mia","Naia","Liria","Zoia"];
...
function AIChatWidget({ t, isMobile }) {
  ...
  const [msgs, setMsgs]       = useState([{
    role:"assistant",
    content:`¡Hola! Soy **${IA_NAME}** 👋\n\nSoy tu asistente financiera de The Big Long. Puedo explicarte cómo funcionan los mercados, instrumentos, tipos de cambio y mucho más.\n\n¿En qué te puedo ayudar hoy?`,
    id: Date.now(),
  }]);
  ...
  const isClientMsg = (txt) =>
    /soy cliente|cliente de m[aá]ximo|cliente tuyo/i.test(txt);
  const isRecoMsg   = (txt) =>
    /recomend[aá]s?|debo comprar|conviene comprar|debo vender|conviene vender|me entr[aá]s?|me sal[ií]s?|qué compro|qué vendo/i.test(txt);
  ...
  const send = async () => {
    ...
    // Special cases — no API needed
    if (isClientMsg(text)) {
      setMsgs([...history, {
        role:"assistant",
        content:`¡Qué bueno que sos cliente de Máximo! 🎉 Te lo aviso directamente — hacé click en el botón y en segundos le llega tu mensaje. Él se va a comunicar con vos.\n\n[WA_ALERT]`,
        id:Date.now()+1,
      }]);
      setLoading(false);
      return;
    ...
    if (isRecoMsg(text)) {
      setMsgs([...history, {
        role:"assistant",
        content:`Eso ya es territorio de Máximo 😊 — las recomendaciones de compra/venta son su especialidad y las da personalmente. La buena noticia: su asesoramiento **no tiene costo**. ¿Lo contactamos?\n\n[WA_CONSULT]`,
        id:Date.now()+1,
      }]);
      setLoading(false);
      return;
    ...
    try {
      ...
      const reply = (data.content||[])
        ...
        .map(b=>b.text)
        .join("") || "Hubo un problema procesando tu consulta. Intentá de nuevo.";
  ...
  const renderContent = (content) => {
    // Render special WA buttons
    if (content.includes("[WA_ALERT]")) {
      const txt = content.replace("[WA_ALERT]","");
      return (
        <div>
          <MiniMarkdown text={txt} />
          <a href={WA_LINK(`Hola Máximo, te escribo desde The Big Long. Soy cliente tuyo y me gustaría consultarte.`)}
            target="_blank" rel="noreferrer"
            style={{ display:"inline-flex", alignItems:"center", gap:8, marginTop:10,
              background:"#25D366", color:"#fff", borderRadius:10, padding:"8px 16px",
              fontFamily:FB, fontSize:12, fontWeight:700, textDecoration:"none" }}>
            💬 Avisarle a Máximo por WhatsApp
          </a>
        </div>
      );
    }
    if (content.includes("[WA_CONSULT]")) {
      const txt = content.replace("[WA_CONSULT]","");
      return (
        <div>
          <MiniMarkdown text={txt} />
          <a href={WA_LINK(`Hola Máximo, te escribo desde The Big Long. Me gustaría que me asesores.`)}
            target="_blank" rel="noreferrer"
            style={{ display:"inline-flex", alignItems:"center", gap:8, marginTop:10,
              background:"#25D366", color:"#fff", borderRadius:10, padding:"8px 16px",
              fontFamily:FB, fontSize:12, fontWeight:700, textDecoration:"none" }}>
            💬 Hablar con Máximo (sin costo)
          </a>
        </div>
      );
    }
  ...
  const MiniMarkdown = ({ text }) => {
    ...
    return (
      <div style={{ fontFamily:FB, fontSize:13, lineHeight:1.7, color:"inherit" }}>
        {lines.map((line, i) => {
          // Bold
          ...
          return <div key={i} style={{ marginBottom:line===""?6:0 }}>{parts}</div>;
        })}
      </div>
    );
  };
    ...
    <>
      ...
      {!open && (
        <button onClick={()=>setOpen(true)} style={{
          position:"fixed", bottom:isMobile?72:28, right:20, zIndex:200,
          width:btnSize, height:btnSize, borderRadius:"50%",
          background:"linear-gradient(135deg,#B0782A,#D4A853)",
          border:"none", cursor:"pointer", boxShadow:"0 4px 20px rgba(176,120,42,.45)",
          display:"flex", alignItems:"center", justifyContent:"center",
          fontSize:24, transition:"transform .2s, box-shadow .2s",
        }}
        onMouseEnter={e=>{ e.currentTarget.style.transform="scale(1.08)"; e.currentTarget.style.boxShadow="0 6px 28px rgba(176,120,42,.6)"; }}
        onMouseLeave={e=>{ e.currentTarget.style.transform="scale(1)"; e.currentTarget.style.boxShadow="0 4px 20px rgba(176,120,42,.45)"; }}>
          🤖
        </button>
      )}
      ...
      {open && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ flex:1 }}>
              <div style={{ fontFamily:FH, fontSize:16, fontWeight:700, color:"#fff", lineHeight:1 }}>{IA_NAME}</div>
              <div style={{ fontFamily:FB, fontSize:10, color:"rgba(255,255,255,.75)", marginTop:2 }}>
                Asistente Financiero · The Big Long
              </div>
            </div>
          ...
          {!auth ? (
            <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:28, gap:16 }}>
              ...
              <div style={{ fontFamily:FH, fontSize:20, fontWeight:700, color:t.tx, textAlign:"center" }}>Acceso privado</div>
              <p style={{ fontFamily:FB, fontSize:12, color:t.mu, textAlign:"center", lineHeight:1.6 }}>
                Esta herramienta es de uso interno. Ingresá la contraseña para continuar.
              </p>
              <input
                type="password"
                ...
                style={{
                  width:"100%", padding:"10px 14px", borderRadius:10, fontFamily:FB, fontSize:14,
                  border:`2px solid ${pwdErr?"#dc2626":t.brd}`, background:t.alt, color:t.tx,
                  outline:"none",
                }}
              />
              {pwdErr && <p style={{ fontFamily:FB, fontSize:11, color:"#dc2626", marginTop:-8 }}>Contraseña incorrecta</p>}
              <button onClick={checkPwd} style={{
                width:"100%", padding:"11px", borderRadius:10, border:"none",
                background:"linear-gradient(135deg,#B0782A,#D4A853)", color:"#fff",
            ...
            <>
              ...
              <div style={{ flex:1, overflowY:"auto", padding:"16px 14px", display:"flex", flexDirection:"column", gap:12 }}>
                  ...
                  <div key={m.id} style={{
                    ...
                    <div style={{
                      ...
                      {m.role==="assistant"
                        ...
                        : <span style={{ fontFamily:FB, fontSize:13, lineHeight:1.6 }}>{m.content}</span>
                      }
                    </div>
                  </div>
                ))}
                {loading && (
                  <div style={{ display:"flex", justifyContent:"flex-start" }}>
                    <div style={{ background:t.alt, border:`1px solid ${t.brd}`, borderRadius:"14px 14px 14px 4px", padding:"10px 16px" }}>
                      <div style={{ display:"flex", gap:5, alignItems:"center" }}>
                        {[0,1,2].map(i => (
                          <div key={i} style={{
                            width:7, height:7, borderRadius:"50%", background:t.go,
              ...
              {msgs.length <= 1 && (
                <div style={{ padding:"0 14px 10px", display:"flex", gap:6, flexWrap:"wrap" }}>
                  {["¿Qué es un LECAP?","¿Cómo funciona el MEP?","¿Qué es el riesgo país?","¿Cómo invierto en dólares?"].map((s,i) => (
                    <button key={i} onClick={()=>{ setInput(s); }} style={{
                      background:t.goBg, border:`1px solid ${t.go}44`, borderRadius:20,
                      padding:"4px 10px", fontFamily:FB, fontSize:10, color:t.go,
              ...
              {/* Input */}
              <div style={{ padding:"10px 14px 14px", borderTop:`1px solid ${t.brd}`, display:"flex", gap:8, flexShrink:0 }}>
                <input
                  value={input}
                  ...
                  disabled={loading}
                  style={{
                    flex:1, padding:"10px 14px", borderRadius:10, fontFamily:FB, fontSize:13,
                    border:`1px solid ${t.brd}`, background:t.alt, color:t.tx, outline:"none",
                  }}
                />
              ...
              <div style={{ padding:"0 14px 10px", textAlign:"center" }}>
                <p style={{ fontFamily:FB, fontSize:9, color:t.fa, lineHeight:1.5 }}>
                  {IA_NAME} puede cometer errores. No constituye asesoramiento de inversión.{" "}
                  <a href={WA_LINK("Hola Máximo, te escribo desde The Big Long.")} target="_blank" rel="noreferrer"
                    style={{ color:t.go, textDecoration:"none", fontWeight:600 }}>
                    Hablar con Máximo →
                  </a>
                </p>
              </div>
