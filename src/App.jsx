...
function AIChatWidget({ t, isMobile }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      const data = await res.json();
      const reply = data.reply || "Hubo un problema procesando tu consulta. Intentá de nuevo.";
      setMsgs([...history, { role:"assistant", content:reply, id:Date.now()+1 }]);
    } catch(err) {
      setMsgs([...history, {
        role:"assistant",
        content:`Tuve un problema técnico momentáneo. Intentá de nuevo en unos segundos. Si el problema persiste, podés contactar directamente a Máximo 👇\n\n[WA_CONSULT]`,
        id:Date.now()+1,
      }]);
    }
    setLoading(false);
